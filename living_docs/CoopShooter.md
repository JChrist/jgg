...
- Static API audit is not enough: Phaser 4 silently removed components like `Arc.geom` and `setLifespan`. The only reliable proof is a runtime interaction test — a CDP harness (touch emulation + dispatched events) caught the FIRE-tap crash that static review missed.
