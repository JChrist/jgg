...
| 1 | 1.7 | Rewrite for Phaser 4 (latest): verify every API against the installed `.d.ts`, replace removed `setLifespan` with timers, runtime-verify headless | M | 🟢 done | Reverses the earlier pin-to-Phaser-3 decision; user requires latest dependencies |
...
| Date | Decision | Why |
|---|---|---|
| 2026-08-02 | Set `input.activePointers: 4` in the game config | Phaser 4 defaults to 1 active touch pointer, so the second finger (FIRE/reload while the joystick holds the only pointer) was silently dropped — "moving and firing together doesn't work". 4 pointers covers two-finger touch for two players in future local co-op. Reproduced with a CDP multi-touch harness (emulated 390x844 phone) and verified: move+fire now works simultaneously |
| 2026-08-02 | Fix FIRE/R tap crash: `Phaser.GameObjects.Arc.geom` no longer exists in Phaser 4 — replace the `Circle.Contains(arc.geom, …)` hit test with a plain distance check on `arc.radius`/`x`/`y` | The v3 API removal made every right-side tap throw a TypeError before `shoot` could be set — the "FIRE does nothing" bug on mobile. Verified end-to-end with a CDP touch harness (emulated 390x844 phone): FIRE tap drops ammo and spawns bullets, joystick drag moves the player |
| 2026-08-02 | Keep a `window.__game` debug hook in main.js | Phaser 4 exposes no global game reference, which made runtime debugging impossible. Harmless and useful for the headless test harness |
| 2026-08-02 | Move to Phaser 4 (latest, `phaser@^4.2.1`), reversing the earlier pin to Phaser 3 | User requires latest dependencies — no starting on versions already superseded. API audit against the installed `types/phaser.d.ts` showed the code is nearly v4-compatible (config `physics` block fixes the boot crash; the removed `setLifespan` replaced with `time.delayedCall`). The real boot crash had been the missing `physics` config all along, not the Phaser version |
| 2026-08-02 | Pin `phaser@^3.90.0` (Phaser 3) — `npm install phaser` had silently pulled Phaser 4, whose API is incompatible with this codebase (`this.physics.world` etc.); also add explicit `physics: { default: 'arcade' }` to the game config | Both bugs were found only via headless browser runtime check: the deployed game threw `Uncaught TypeError: reading 'world'` at boot on every device, so the screen stayed on the background color. `npm run build` had been passing the whole time because it only compiles, never executes. **Note: this decision is reversed by the Phaser 4 entry above** |
...
- The bundled `types/phaser.d.ts` of an installed package is the authoritative API reference — auditing it (config, physics factory, removed components) made the Phaser 4 migration a two-line change instead of a guess-and-pray rewrite.
- Static API audit is not enough: Phaser 4 silently removed components like `Arc.geom` and `setLifespan`, and its default of 1 active touch pointer breaks multi-touch. The only reliable proof is a runtime interaction test — a CDP harness (touch emulation + dispatched events) caught the FIRE-tap crash and the multi-touch drop that static review missed.
- CDP harness details that bite: `Input.dispatchTouchEvent` `touchEnd` takes the points being *removed*; the full active-touch list must be sent on every `touchStart`; and coordinates are CSS pixels, not game pixels (FIT-scale must be inverted). A wrong assumption here produces fake bugs (the "joystick dies on FIRE lift" ghost).
- Test harnesses must control game state: the player dies in ~5 seconds when idle, which made every earlier input test run against a corpse. Restart the game inside the harness before testing input.
- Headless Chrome's window size is not what `--window-size` promises (500x699 in practice); FIT-scale screenshots and taps need the real canvas rect (`getBoundingClientRect`).
